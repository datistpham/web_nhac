...
import { ComponentSearchResult } from '../Search/SearchResultRebuild/SearchResultRebuild';

const cx = classNames.bind(styles);

function RightSidebar() {
    const {user, change }= useContext(AppContext)
    ...
    return (
        <div className={cx('wrapper')}>
            <div className={cx('header')}>
                <h1 className={cx('title')} style={{fontSize: 20, marginBottom: 12}}>Danh sách bài hát nghe gần đây</h1>
            </div>
            <div className={cx('content')}>
                <div className={cx('next-list')}>
                    ...
                    </div> */}
                    {
                        data?.map((item, key)=> 
                        // đa hình
                        <ComponentSearchResult {...item} key={key} />)
                    }
                    {/* {isRandom
