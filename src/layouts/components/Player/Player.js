...
function Player() {
    ...
    const audioRef = useRef(); // important
    const volumeRef = useRef();
    const radioRef = useRef();
    const track_name= useSelector(state=> state.audio.track_name)
    const artist_name= useSelector(state=> state.audio.artist_name)
    const [dragSong, setDragSong]= useState(true)
        ...
        <div className={cx('container', isFull && 'full')}>
            <div className={cx('full-player')}>
                ...
                <div className={cx('info-full')}>
                    <h3 className={cx('title-full')}>{track_name}</h3>
                    <p className={cx('artists-full')}>{artist_name}</p>
                </div>
            </div>
            <div
            ...
            >
                <div className={cx('info')}>
                    ...
                    <div className={cx('name')}>
                        <h3 style={{marginBottom: 8}} className={cx('title')}>{track_name || 'Tên bài hát'}</h3>
                        <p className={cx('artists')}>
                            {artist_name || songInfo.activeUsers + ' người đang nghe'}
                        </p>
                    </div>
                </div>
                {/*  */}
                <div className={cx('control')}>
                    ...
                    {songInfo.status === 2 ? (
                        ...
                        <div className={cx('range')} style={{justifyContent: "space-between"}}>
                            ...
                            <div onMouseLeave={()=> setDragSong(true)} onMouseEnter={()=> setDragSong(false)} style={{width: 286, display: "flex", justifyContent: "center", alignItems: 'center', position: 'relative', overflow: "hidden"}}>
                                <input
                                ...
                                {
                                    dragSong=== true && 
                                <div style={{width: `${parseInt(currentTime) / parseInt(duration) * 100}%`, position: "absolute", top: 0, left: 0, height: "100%", background: "#2e89ff", borderRadius: 80}}></div>
                                }
                            </div>
